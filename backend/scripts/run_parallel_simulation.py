...
def _get_post_info(
    ...
    """
    try:
        # Performance optimization: Fetch user name details in primary query via JOIN
        # Expected Impact: Avoids N+1 query problem, significantly reducing database hits
        # from O(N) to O(1) per post context fetch loop.
        cursor.execute("""
...
def _get_comment_info(
    ...
    try:
        # Performance optimization: Fetch user name details in primary query via JOIN
        # Expected Impact: Avoids N+1 query problem, significantly reducing database hits
        # from O(N) to O(1) per comment context fetch loop.
        cursor.execute("""
