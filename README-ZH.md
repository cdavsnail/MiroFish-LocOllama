...
| 工具 | 版本要求 | 说明 | 安装检查 |
|------|---------|------|---------|
| **Node.js** | 18+ | 前端运行环境，包含 pnpm | `node -v` |
| **pnpm** | 最新版 | 前端包管理器 | `pnpm --version` |
...
# 默认配置为使用本地 Ollama 实例进行零成本运行
LLM_API_KEY=ollama
LLM_BASE_URL=http://localhost:11434/v1
LLM_MODEL_NAME=qwen-plus
```

> **关于图谱记忆的说明：** MiroFish 现在使用基于本地 JSON 的图谱系统（`LocalGraph` / `ZepMock`）进行记忆和实体抽取。不再需要 Zep Cloud。
...
```bash
pnpm run backend   # 仅启动后端
pnpm run frontend  # 仅启动前端
```

#### 4. 测试

要运行后端测试，请导航到 `backend` 目录并通过 uv 运行 pytest：
